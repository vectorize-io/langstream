...
class MSSharepointSourceIT extends AbstractGenericStreamingApplicationRunner {
    ...
    public void test() throws Exception {
        ...
        Site selectedSite = sites.stream().filter(site -> SITE_ID.equals(site.getId()) || SITE_ID.equals(site.getName())).findFirst().orElseThrow(() -> new IllegalStateException("Site not found"));
        ...
        List<DriveItem> children = client.drives().byDriveId(drive.getId()).items().byDriveItemId(rootItem.getId()).children().get().getValue();
        ...
        String folderId = client.drives().byDriveId(drive.getId()).items().byDriveItemId(rootItem.getId()).children().post(folderItem).getId();
        ...
        List<byte[]> docs = List.of(
                ...
                MSSharepointSourceIT.class.getResourceAsStream("/doc2.docx").readAllBytes()
        );
        ...
        for (int i = 0; i < 2; i++) {
            ...
            DriveItem item = client.drives().byDriveId(drive.getId()).items().byDriveItemId(folderId).children().post(driveItem);

            client.drives().byDriveId(drive.getId()).items().byDriveItemId(item.getId()).content().put(new ByteArrayInputStream(bytes));
            ...
            try (TopicConsumer deletedDocumentsConsumer = createConsumer("deleted-objects");
                ...
                waitForMessages(
                        ...
                        (consumerRecords, objects) -> {
                            ...
                            assertTrue(((String)consumerRecords.get(0).value()).contains("This is a document"));
                            assertEquals(fileIds.get(1), consumerRecords.get(1).key());
                            assertTrue(((String)consumerRecords.get(1).value()).contains("This is a another document"));
                        });


                client.drives().byDriveId(drive.getId()).items().byDriveItemId(itemIds.get(0)).delete();
