...
public class ElasticSearchVectorDatabaseWriterConfig
    ...
    public static final ElasticSearchVectorDatabaseWriterConfig INSTANCE =
            new ElasticSearchVectorDatabaseWriterConfig();

    @Override
    public Class getAgentConfigModelClass() {
        return ElasticSearchVectorDatabaseWriterConfig.class;
    }
    ...
    public static class BulkParameters {
        ...
        @ConfigProperty(
                description =
                        """
                Specifies the number of active shards that must be available before ElasticSearch processes the bulk request. Default is 1 (only the primary shard). Set to all or a positive integer. Values greater than 1 require replicas. For example, if you specify a value of 3, the index must have two replicas distributed across two additional nodes for the request to succeed.
                Refer to the ElasticSearch documentation for more details.
                """)
    ...
    @ConfigProperty(
            description =
                    "JSTL Expression to compute the index _id field. Leave it empty to let ElasticSearch auto-generate the _id field.")
    String id;

    @ConfigProperty(description = "ElasticSearch bulk URL parameters.")
